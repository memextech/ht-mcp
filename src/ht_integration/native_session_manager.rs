...
impl NativeSessionManager {
    ...
    pub async fn create_session(&self, args: CreateSessionArgs) -> Result<serde_json::Value> {
        ...
        let session_id = manager.create_session(command.clone(), enable_web_server).await
    ...
    pub async fn send_keys(&self, args: SendKeysArgs) -> Result<serde_json::Value> {
        let mut manager = self.ht_manager.lock().await;
        manager.send_keys(&args.session_id, args.keys.clone()).await
    ...
    pub async fn take_snapshot(&self, args: TakeSnapshotArgs) -> Result<serde_json::Value> {
        let mut manager = self.ht_manager.lock().await;
        let snapshot = manager.take_snapshot(&args.session_id).await
    ...
    pub async fn execute_command(&self, args: ExecuteCommandArgs) -> Result<serde_json::Value> {
        let mut manager = self.ht_manager.lock().await;
        let output = manager.execute_command(&args.session_id, &args.command).await
    ...
    pub async fn list_sessions(&self) -> Result<serde_json::Value> {
        ...
        let session_list: Vec<serde_json::Value> = sessions.iter()
            .map(|session| {
    ...
    pub async fn close_session(&self, args: CloseSessionArgs) -> Result<serde_json::Value> {
        ...
        manager.close_session(&args.session_id).await
...
mod tests {
    ...
    async fn test_native_session_manager() {
        ...
        // Create a session
        let create_result = manager.create_session(CreateSessionArgs {
        ...
        // Test snapshot
        let snapshot_result = manager.take_snapshot(TakeSnapshotArgs {
            session_id: session_id.clone(),
        }).await.unwrap();
        
        assert!(snapshot_result["snapshot"].as_str().unwrap().contains("session"));
        
        // Test command execution
        let exec_result = manager.execute_command(ExecuteCommandArgs {
            session_id: session_id.clone(),
